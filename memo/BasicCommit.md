...
GitHub Desktopでは、以下のような基本的なコミットの整理は可能です。細かすぎる更新が続き（似たようなCommitが多くなる）、履歴管理としては複雑になりすぎる場合には、高度な履歴編集（コミットの統合 Squash や Push 後の並べ替え）が必要となりますが、まずはそのようなことを扱う前に、以下の3点を実行できるようにしておくとよいでしょう（Squash等はその後で問題ありません）。
